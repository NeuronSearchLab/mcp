...
function formatPlatformApiResponse(method, path, res) {
...
function formatEventBreakdown(rows) {
    if (!rows.length)
        return '  No events recorded.';
    return rows.map((row) => `  ${row.event_name}: ${row.count}`).join('\n');
}
function formatItemAnalytics(res, itemId) {
    ...
    if (res?.last_served_at)
        lines.push(`Last served: ${res.last_served_at}`);
    if (res?.last_event_at)
        lines.push(`Last event: ${res.last_event_at}`);
    lines.push('', 'Event breakdown:', formatEventBreakdown(res?.event_breakdown ?? []));
    return lines.join('\n');
}
function formatUserAnalytics(res, userId) {
...
function formatItemComparison(res) {
...
function formatTopItemsFallback(primaryRes, fallbackRes) {
...
const TOOLS = [
    // ── API tools ───────────────────────────────────────────────────────
    {
        ...
        description: 'Fetch personalised recommendations for a user. Returns ranked items with scores. ' +
    ...
    },
    {
        ...
        description: 'Fetch the next auto-generated recommendation section for a user. ' +
    ...
    {
        name: 'upsert_item',
        description: 'Add or update an item in the recommendation catalogue. ' +
    ...
    {
        name: 'delete_items',
        description: 'Permanently remove one or more items from the recommendation catalogue. ' +
    ...
    {
        name: 'search_items',
        description: 'Search the recommendation catalogue by keyword. ' +
    ...
    },
    {
        ...
        description: 'Explain why a specific item was (or would be) ranked at a particular position for a user. ' +
    ...
    // ── Platform management tools ───────────────────────────────────────
    {
        ...
        description: 'Get the authenticated team\'s current billing plan, resolved resource limits, live resource counts, metered usage, and exact overages. ' +
    ...
    {
        name: 'list_contexts',
        description: 'List all recommendation contexts (feeds) configured for your team. ' +
            'Contexts define how recommendations are scoped, filtered, and grouped.',
        inputSchema: { type: 'object', properties: {} },
    },
    {
        name: 'create_context',
        description: 'Create a new recommendation context (feed). ' +
    ...
    {
        name: 'list_pipelines',
        description: 'List all ranking pipelines. ' +
    ...
    },
    {
        ...
        description: 'Create a new ranking pipeline with default stages. ' +
    ...
    {
        name: 'list_rules',
        description: 'List ranking rules, optionally filtered by context. ' +
    ...
    {
        name: 'create_rule',
        description: 'Create a new ranking rule. Rule types: ' +
    ...
    {
        name: 'create_segment',
        description: 'Create a new user segment with membership conditions. ' +
    ...
    },
    {
        ...
        description: 'Create a new A/B experiment comparing pipeline variants. ' +
    ...
    },
    {
        ...
        description: 'Create a time-bounded campaign that injects ranking rules between a start and end datetime. ' +
    ...
    // ── Analytics / Metrics tools ──────────────────────────────────────
    {
        ...
        description: 'Get ranking performance metrics for a pipeline or context: CTR by position, overall CTR, conversion rate, diversity score, catalogue coverage, and revenue per session.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Fallback tool for internal mode only. Calls safe admin-console /api routes directly when no dedicated MCP tool exists. ' +
        ...
        try {
            switch (name) {
                ...
                case 'update_context': {
                    ...
                    const listRes = await client.get('/api/context');
                    const contexts = Array.isArray(listRes?.contexts) ? listRes.contexts : [];
                    const existing = contexts.find((entry) => Number(entry.id) === input.context_id);
                    if (!existing) {
                ...
                case 'delete_context': {
                    ...
                    const res = await client.delete(`/api/context/${input.context_id}`);
                ...
                case 'get_context': {
                    ...
                    const res = await client.get('/api/context');
                    const contexts = Array.isArray(res?.contexts) ? res.contexts : [];
                    const context = contexts.find((entry) => Number(entry.id) === input.context_id);
                    if (!context) {
                        return { content: [{ type: 'text', text: `❌ Context ${input.context_id} not found.` }], isError: true };
                    }
                ...
                case 'clone_pipeline': {
                    ...
                    const source = await client.get(`/api/pipelines/${input.pipeline_id}`);
                ...
                case 'list_rules': {
                    ...
                    if (input.context_id !== undefined)
                        query.contextId = input.context_id;
                ...
                case 'get_training_job': {
                    ...
                    const res = await client.get('/api/training-jobs');
                    const jobs = Array.isArray(res?.jobs) ? res.jobs : [];
                    const job = jobs.find((entry) => String(entry.id) === input.job_id
                ...
                case 'cancel_training_job': {
                    ...
                    const listRes = await client.get('/api/training-jobs');
                ...
                case 'get_ranking_metrics': {
                    ...
                    const res = mode === 'internal'
                        ? await client.get('/api/analytics', {
                            preset: input.window === '1d' ? '24h' : input.window,
                            context: input.context_id,
                        })
                        : await client.get('/analytics/ranking', {
                            pipeline_id: input.pipeline_id,
                            context_id: input.context_id,
                            window: input.window,
                        });
                    ...
                    if (res.ctr != null)
                        lines.push(`  Overall CTR: ${(res.ctr * 100).toFixed(2)}%`);
                    if (res.conversion_rate != null)
                        lines.push(`  Conversion rate: ${(res.conversion_rate * 100).toFixed(2)}%`);
                    if (res.diversity_score != null)
                        lines.push(`  Diversity score: ${res.diversity_score.toFixed(3)}`);
                    if (res.coverage != null)
                        lines.push(`  Catalogue coverage: ${(res.coverage * 100).toFixed(1)}%`);
                    if (res.revenue_per_session != null)
                        lines.push(`  Revenue per session: $${res.revenue_per_session.toFixed(2)}`);
                ...
                case 'get_user_analytics': {
                    ...
                    const res = await client.get(`/api/analytics/users/${encodeURIComponent(input.user_id)}`, {
                        window: input.window,
                        context_id: input.context_id,
                    });
                ...
                case 'get_item_analytics': {
                    ...
                    const res = await client.get(`/api/analytics/items/${encodeURIComponent(input.item_id)}`, {
                        window: input.window,
                        context_id: input.context_id,
                    });
                ...
                case 'compare_items': {
                    ...
                    const res = await client.get('/api/analytics/items/compare', {
                ...
                case 'top_items': {
                    ...
                    const res = await client.get('/api/analytics/top-items', {
                        metric: input.metric,
                    ...
                    if (!hasPrimaryItems && input.metric === 'events' && !input.event_name && input.event_id == null) {
                        const fallbackRes = await client.get('/api/analytics/top-items', {
                            metric: 'served',
                            window: input.window,
                            context_id: input.context_id,
                            limit: input.limit,
                ...
                case 'call_platform_api': {
                    ...
                    const query = input.query
                        ? Object.fromEntries(Object.entries(input.query).map(([key, value]) => [
                            key,
                    ...
                    switch (input.method) {
                        case 'GET':
                            res = await client.get(input.path, query);
                            break;
                        case 'POST':
                            res = await client.post(input.path, input.body ?? {});
                            break;
                        case 'PATCH':
                            res = await client.patch(input.path, input.body ?? {});
                            break;
                        case 'PUT':
                            res = await client.put(input.path, input.body ?? {});
                            break;
                        case 'DELETE':
                            res = await client.delete(input.path, input.body ?? {});
                            break;
                        default:
                            return {
